...
export default function PickUp({ setPickBook, distance }) {
	...
	return (
		<View className="absolute bottom-0 w-full px-[15px] flex flex-col justify-center items-center bg-white pt-[20px]">
				...
				<View className="flex flex-row justify-between border-[#EFEFEF] border-b-[1.5px]">
					<View className="flex flex-col justify-center w-[240px] max-w-[262px]">
						...
						<Text className="text-[13px]">
							{distance + "km"} . Pick up/Drop off Gate, 227
							Nguyễn
						</Text>
					</View>
			...
			{!addPickUp ? (
				<Pressable
					onPress={() => setAddPickUp(true)}
					className="flex flex-row items-center justify-start min-w-[350px] pl-[15px] py-[20px] border-[#EFEFEF] border-b-[1.5px] mb-[20px]"
				>
					<Entypo name="edit" size={24} color="#01B0E7" />
					<Text className="text-[#01B0E7] ml-[13px] font-bold">
				...
				<View className="min-w-[350px] border-[#EFEFEF] border-b-[1.5px] mb-[20px]">
					<View className="flex flex-row min-w-[350px] pl-[15px] py-[10px] mt-[20px] rounded-[8px] border-[#2EAF69] border-[1.5px] mb-[20px]">
						...
						<TextInput
							placeholder="Add pick-up notes for driver"
							className="font-normal ml-[10px] min-w-[300px]"
						/>
					</View>
				</View>
			)}
			<Pressable
				onPress={() => setPickBook((prev) => !prev)}
				className="bg-[#00B14F] min-w-[350px] py-[15px] rounded-[8px] flex justify-center items-center mb-[15px]"
			>
