...
export const calculateDistance = async ({ origin, destination }) => {
	...
	const result = await axios.get(
		`https://maps.googleapis.com/maps/api/distancematrix/json?origins=${org}&destinations=${des}&key=${EXPO_PUBLIC_MAP_APIKEY}`
	);

	return (result.data.rows[0].elements[0].distance.value / 1000).toFixed(1);
};
